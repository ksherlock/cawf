...
#define Charput(s) { \
	if (!Divert && Pageprt) \
		{ (void) fwrite((char *)s, 1, 1, stdout); } }

_PROTOTYPE(unsigned char *Asmcode,(unsigned char **s, unsigned char *c));
_PROTOTYPE(int Asmname,(unsigned char *s, unsigned char *c));
_PROTOTYPE(char *AsmPg,(char *s, int *n));
_PROTOTYPE(int AsmPgRange,(char *s));
_PROTOTYPE(void Delmacro,(int mx));
_PROTOTYPE(int Defdev,(void));
_PROTOTYPE(void Delnum,(int nx));
_PROTOTYPE(void Delstr,(int sx));
_PROTOTYPE(void Endword,(void));
...
_PROTOTYPE(unsigned char *Newstr,(unsigned char *s));
_PROTOTYPE(int PageInRange,(int pg));
_PROTOTYPE(void Pass2,(unsigned char *line));
_PROTOTYPE(void Pass3,(int len, unsigned char *word, int wx, unsigned char *sarg, int narg));
...
_PROTOTYPE(void Stringput,(unsigned char *s, int len));
